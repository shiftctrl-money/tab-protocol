...
/// @dev Refer https://github.com/makerdao/dss-vest
abstract contract DssVest {
    ...
    uint256 public constant THIRTY_YEARS = 30 * 365 days;
    ...
    function file(bytes32 what, uint256 data) external lock auth {
        if (what == "cap") {
            cap = data;
        } // The maximum amount of tokens that can be streamed per-second per vest
        else {
            revert("DssVest/file-unrecognized-param");
        }
        emit File(what, data);
    }
        ...
        returns (uint256 id)
    {
        ...
        require(_bgn < add(block.timestamp, THIRTY_YEARS), "DssVest/bgn-too-far");
        require(_bgn > sub(block.timestamp, THIRTY_YEARS), "DssVest/bgn-too-long-ago");
        require(_tau > 0, "DssVest/tau-zero");
        require(_tot / _tau <= cap, "DssVest/rate-too-high");
        require(_tau <= THIRTY_YEARS, "DssVest/tau-too-long");
    ...
    function _yank(uint256 _id, uint256 _end) internal lock {
        ...
        if (_end < _award.fin) {
            ...
            if (end < _award.bgn) {
                ...
                awards[_id].tot =
                    toUint128(add(unpaid(_end, _award.bgn, _award.clf, _award.fin, _award.tot, _award.rxd), _award.rxd));
            }
        }
